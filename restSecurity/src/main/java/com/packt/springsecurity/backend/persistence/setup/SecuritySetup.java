...
public class SecuritySetup implements ApplicationListener<ContextRefreshedEvent> {
	...
	public final void onApplicationEvent(final ContextRefreshedEvent event) {
		if (!setupDone) {
			...
			SecurityContextHolder.getContext().setAuthentication(new UsernamePasswordAuthenticationToken(
					"admin_setup_principal", "pass", Lists.newArrayList(new SimpleGrantedAuthority("ROLE_ADMIN"))));
	...
	final void createUsers() {
		...
		createUserIfNotExisting(SecurityConstants.ADMIN1_USERNAME, SecurityConstants.ADMIN1_PASS,
				Sets.<Authority>newHashSet(authorityUser, authorityAdmin));
		createUserIfNotExisting(SecurityConstants.ADMIN2_USERNAME, SecurityConstants.ADMIN2_PASS,
				Sets.<Authority>newHashSet(authorityUser, authorityAdmin));
		createUserIfNotExisting(SecurityConstants.USER1_USERNAME, SecurityConstants.USER1_PASS,
				Sets.<Authority>newHashSet(authorityUser));
	}
